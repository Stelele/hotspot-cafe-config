...
1. Log in to daloRADIUS at `http://<your-server>:8082` (default: `administrator` / `REDACTED`)
...
4. Fill in:
   ...
   - **NAS Type**: `other`
   - **Secret**: The shared secret you configured in `RADIUS_SECRET` (e.g., `REDACTED_SECRET` or your custom secret)
...
```bash
docker exec daloradius radtest <username> <password> localhost 0 REDACTED_SECRET
```
